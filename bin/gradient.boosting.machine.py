"""Gradient Boosting Machine classifier for SLE classification.

Trains a GBM model using features selected by Boruta and/or Elastic Net,
evaluates on a held-out test set, and saves metrics, plots, and the trained model.

Usage:
    python gradient.boosting.machine.py <input_RDS_file> <split_number> <feature_type>
...
metrics.to_csv(f'pseudobulk_update/split_{sys.argv[2]}/{sys.argv[3]}/metrics/GBM_metrics_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)

# Save confusion matrix to file
confusion = pd.DataFrame(confusion_matrix(y_test, y_pred))
confusion.to_csv(f'pseudobulk_update/split_{sys.argv[2]}/{sys.argv[3]}/metrics/GBM_confusion_'+os.path.basename(file).replace('.RDS', '')+'.csv', index=False)
...
# Set the title
ax.set_title('GBM: ' + os.path.basename(file).replace('.RDS', '').replace('.', ' '))
...
plt.tight_layout()
# Save the figure
plt.savefig(f'pseudobulk_update/split_{sys.argv[2]}/{sys.argv[3]}/confusion/GBM_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')
plt.close()
...
plt.savefig(f'pseudobulk_update/split_{sys.argv[2]}/{sys.argv[3]}/AUROC/GBM_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')
...
plt.savefig(f'pseudobulk_update/split_{sys.argv[2]}/{sys.argv[3]}/PRC/GBM_'+os.path.basename(file).replace('.RDS', '')+'.pdf', bbox_inches='tight')

# Save the model
filename = f'pseudobulk_update/split_{sys.argv[2]}/{sys.argv[3]}/ML.models/GBM_model_'+os.path.basename(file).replace('.RDS', '')+'.sav'
